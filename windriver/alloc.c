...
{
	PHYSICAL_ADDRESS LowAddress; /* all under 16MB, reserved for DMA */
	PHYSICAL_ADDRESS HighAddress;
	PHYSICAL_ADDRESS SkipBytes;

    LowAddress.QuadPart = 0x1000000;
    HighAddress.QuadPart = osdep->hostmem_max_physical_address;
...
void co_os_put_page(struct co_manager* manager, co_pfn_t pfn)
{
	...
	list = &osdep->pages_hash[PFN_HASH(pfn)];

	co_list_each_entry(pfn_ptr, list, node, co_os_pfn_ptr_t) {
		if (pfn_ptr->pfn != pfn)
			continue;
