...
#include <colinux/os/current/kernel/conet.h>

// #define CONET_DEBUG

#ifdef CONET_DEBUG
# define conet_debug(fmt, ...) co_debug_lvl(network, 10, fmt, __VA_ARGS__)
#else
# define conet_debug(fmt, ...)
#endif
#define conet_err_debug(fmt, ...) co_debug_lvl(network, 3, fmt, __VA_ARGS__)

// Forward reference
static void co_conet_proto_transfer_complete(
...
static VOID co_conet_transfer_message_routine(PDEVICE_OBJECT DeviceObject, PVOID Context)
{
...
static void co_conet_free_adapter(conet_adapter_t *adapter)
{
	...
	if ( adapter ) {
		...
		co_list_each_entry_safe(packet, next_packet, &adapter->pending_transfer_list, list_node, conet_packet_t) {
		...
		co_list_each_entry_safe(packet, next_packet, &adapter->pending_send_list, list_node, conet_packet_t) {
			co_list_del(&packet->list_node);
			conet_debug("Unchain and free Buffers (Packet = %p)", packet);
			co_FreeBuffersAndPacket((PNDIS_PACKET)packet);
		}
...
static void co_conet_proto_bind_adapter(
...
static void co_conet_proto_unbind_adapter(
	OUT PNDIS_STATUS	Status,
	IN  NDIS_HANDLE		ProtocolBindingContext,
...
static void co_conet_proto_open_adapter_complete(
	IN NDIS_HANDLE		ProtocolBindingContext,
	IN NDIS_STATUS		Status,
...
static void co_conet_proto_close_adapter_complete(
	IN NDIS_HANDLE		ProtocolBindingContext,
	IN NDIS_STATUS		Status
	)
{
...
static NDIS_STATUS co_conet_proto_receive(
	IN NDIS_HANDLE		ProtocolBindingContext,
	...
	)
{
	...
	if ( LookAheadBufferSize == PacketSize ) {
		...
		message = (struct conet_message *)co_os_malloc(sizeof(struct conet_message) + HeaderBufferSize + LookAheadBufferSize - 1);
		if ( !message ) {
...
static void co_conet_proto_receive_complete(
	IN NDIS_HANDLE		ProtocolBindingContext
	)
{
#ifdef CONET_DEBUG
...
static void co_conet_proto_request_complete(
	IN NDIS_HANDLE		ProtocolBindingContext,
...
static void co_conet_proto_send_complete(
	IN NDIS_HANDLE		ProtocolBindingContext,
	IN PNDIS_PACKET		Packet,
	IN NDIS_STATUS		Status
	)
...
static void co_conet_proto_reset_complete(
	IN NDIS_HANDLE		ProtocolBindingContext,
	IN NDIS_STATUS		Status
	)
{
...
static void co_conet_proto_status(
	IN NDIS_HANDLE		ProtocolBindingContext,
	IN NDIS_STATUS		Status,
...
static void co_conet_proto_status_complete(
	IN NDIS_HANDLE		ProtocolBindingContext
	)
{
	conet_adapter_t		*adapter = (conet_adapter_t*)ProtocolBindingContext;
	...
	)
{
	...
	if ( !co_conet_proto_filter_packet(adapter, packet->packet_buffer) ) {
		...
		message = (struct conet_message *)co_os_malloc(sizeof(struct conet_message) + TotalPacketLength - 1);
		if ( !message ) {
...
static int co_conet_proto_receive_packet(
	IN NDIS_HANDLE		ProtocolBindingContext,
	IN PNDIS_PACKET		Packet
	)
{
...
static NDIS_STATUS co_conet_proto_pnp_handler(
	IN NDIS_HANDLE		ProtocolBindingContext,
...
co_rc_t co_conet_unregister_protocol(co_monitor_t *monitor)
{
	...
	if ( osdep->conet_protocol ) {
		...
		co_list_each_entry_safe(adapter, adapter_next, &osdep->conet_adapters, list_node, conet_adapter_t) {
...
co_rc_t co_conet_bind_adapter(co_monitor_t *monitor, int conet_unit, char *netcfg_id, int promisc, char macaddr[6])
{
	...
	co_os_mutex_acquire(osdep->conet_mutex);
	co_list_each_entry(adapter, &osdep->conet_adapters, list_node, conet_adapter_t) {
